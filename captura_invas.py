...
def extraer_semana_actual(driver):
    """Extrae la tabla de tareas procesadas por usuario en semana actual."""
    filas = []
    try:
        paneles = driver.find_elements(By.CSS_SELECTOR, ".panel.reportPanel, .reportPanel")
        for panel in paneles:
            texto = panel.text
            if "SEMANA ACTUAL" in texto.upper() or "TAREAS PROCESADAS POR USUARIO" in texto.upper():
                rows = panel.find_elements(By.CSS_SELECTOR, "tbody tr")
                for row in rows:
                    celdas = [td.text.strip() for td in row.find_elements(By.TAG_NAME, "td")]
                    if len(celdas) >= 5:
                ...
                break
        if not filas:
            log.warning("  Tabla semana actual no encontrada")
    except Exception as e:

"""
captura_invas_anf.py — Antofagasta
===================================
INSTALACIÓN:
    python -m pip install selenium webdriver-manager

EJECUCIÓN:
    python captura_invas_anf.py
"""
...
# ─────────────────────────────────────────────
INVAS_URL   = "https://serviallinvas.impruvex.com/invaswmsbi/dashboard/3a68d8aa-1363-4b0c-a384-a3b8efacccb5/"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("captura_invas_anf.log", encoding="utf-8"),
...
def login(driver):
    log.info("Navegando a Invas ANF...")
    ...
    if "login" in url or "signin" in url or "account" in url:
        ...
        time.sleep(6)
    # Forzar siempre la URL de Antofagasta
    if driver.current_url.rstrip('/') != INVAS_URL.rstrip('/'):
        log.info("Forzando navegacion a dashboard ANF...")
        driver.get(INVAS_URL)
        time.sleep(6)
...
def extraer_top5_operadores(driver):
    ...
    try:
        ...
        for panel in paneles:
            texto = panel.text
            if "TOP 5" in texto.upper() or "OPERADORES TAREAS" in texto.upper():
                ...
                log.info(f"  TOP 5: {len(filas)} filas")
                break
    except Exception as e:
...
def extraer_semana_actual(driver):
    filas = []
    try:
        paneles = driver.find_elements(By.CSS_SELECTOR, ".panel.reportPanel, .reportPanel")
        for panel in paneles:
            texto = panel.text
            if "SEMANA ACTUAL" in texto.upper():
                rows = panel.find_elements(By.CSS_SELECTOR, "tbody tr")
                for row in rows:
                    celdas = [td.text.strip() for td in row.find_elements(By.TAG_NAME, "td")]
                    if len(celdas) >= 5:
                ...
                log.info(f"  Semana actual: {len(filas)} filas")
                break
    except Exception as e:
        log.warning(f"  Error semana actual: {e}")
    return filas
...
def main():
    log.info("=" * 50)
    log.info("Iniciando extraccion ANF — " + datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
    log.info("=" * 50)
    ...
    try:
        ...
        datos = {
            ...
            "fillrate_mensual": extraer_grafico(driver, "FILLRATE MENSUAL"),
            "top5_operadores":  extraer_top5_operadores(driver),
            "semana_actual":    extraer_semana_actual(driver),
            "ocupacion":        extraer_tabla_ocupacion(driver),
        }

        ruta_json = os.path.join(CARPETA, "datos_anf.json")
        with open(ruta_json, "w", encoding="utf-8") as f:
            json.dump(datos, f, ensure_ascii=False, indent=2)

        log.info("datos_anf.json guardado.")
        log.info("Extraccion ANF completada exitosamente.")
